// Dafny program Lab3_circular_buffer_template.dfy compiled into C#
...
// Command Line Options: c:\Users\nauma\Desktop\DV\TestingDebugging\lab4\Lab3_circular_buffer_template.dfy /verifyAllModules /spillTargetCode:1 /compile:3 /out:bin\Lab3_circular_buffer_template
// Lab3_circular_buffer_template.dfy
...
class CircularMemory {
    ...
    decreases length
  {
    ...
    while i < length
      ...
      invariant arrayOfZeroes.Length == length
  ...
  {
    var tmpReadPosition := read_position;
    var tmpWritePosition := write_position;
    ...
    while i < oldArrayLength
      ...
      invariant cells.Length > oldArrayLength
      invariant 0 <= i <= cells.Length
      invariant cells.Length == 2 * old(cells.Length)
...
namespace _module {
  ...
  public partial class CircularMemory {
    public CircularMemory() {
      this.cells = new BigInteger[0];
      this.read__position = BigInteger.Zero;
      this.write__position = BigInteger.Zero;
      this.isFlipped = false;
    }
    public  BigInteger[] cells {get; set;}
    public  BigInteger read__position {get; set;}
    public  BigInteger write__position {get; set;}
    public  bool isFlipped {get; set;}
    public void Init(BigInteger cap)
    {
      BigInteger[] _nw0 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked(cap, "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      (this).cells = _nw0;
      BigInteger _rhs0 = BigInteger.Zero;
      BigInteger _rhs1 = BigInteger.Zero;
      CircularMemory _lhs0 = this;
      CircularMemory _lhs1 = this;
      _lhs0.read__position = _rhs0;
      _lhs1.write__position = _rhs1;
      (this).isFlipped = false;
    }
    public void Read(out bool isSuccess, out BigInteger content)
    {
      isSuccess = false;
      content = BigInteger.Zero;
      if (this.isFlipped) {
        isSuccess = true;
        content = (this.cells)[(int)(this.read__position)];
        (this).read__position = (this.read__position) + (BigInteger.One);
        if ((this.read__position) == (new BigInteger((this.cells).Length))) {
          (this).isFlipped = false;
          (this).read__position = BigInteger.Zero;
        }
      } else {
        if ((this.read__position) == (this.write__position)) {
          isSuccess = false;
          content = BigInteger.Zero;
        } else {
          isSuccess = true;
    ...
    public bool Write(BigInteger input)
    {
      bool isSuccess = false;
      if (!(this.isFlipped)) {
        isSuccess = true;
        var _arr0 = this.cells;
        var _index0 = this.write__position;
        _arr0[(int)(_index0)] = input;
        (this).write__position = (this.write__position) + (BigInteger.One);
        if ((this.write__position) == (new BigInteger((this.cells).Length))) {
          (this).write__position = BigInteger.Zero;
          (this).isFlipped = true;
        }
      } else {
        if ((this.write__position) == (this.read__position)) {
          isSuccess = false;
        } else {
          isSuccess = true;
          var _arr1 = this.cells;
          var _index1 = this.write__position;
          _arr1[(int)(_index1)] = input;
          (this).write__position = (this.write__position) + (BigInteger.One);
        }
      }
      return isSuccess;
    }
    public BigInteger[] createArrayOfZeroes(BigInteger length)
    {
      BigInteger[] arrayOfZeroes = new BigInteger[0];
      BigInteger[] _nw1 = new BigInteger[Dafny.Helpers.ToIntChecked(Dafny.Helpers.ToIntChecked(length, "C# arrays may not be larger than the max 32-bit integer"),"C# array size must not be larger than max 32-bit int")];
      arrayOfZeroes = _nw1;
      BigInteger _0_i;
      _0_i = BigInteger.Zero;
      while ((_0_i) < (length)) {
        (arrayOfZeroes)[(int)((_0_i))] = BigInteger.Zero;
    ...
    {
      BigInteger _1_tmpReadPosition;
      _1_tmpReadPosition = this.read__position;
      BigInteger _2_tmpWritePosition;
      _2_tmpWritePosition = this.write__position;
      BigInteger _3_oldArrayLength;
      _3_oldArrayLength = new BigInteger((this.cells).Length);
      BigInteger[] _4_oldArray;
      _4_oldArray = this.cells;
      BigInteger[] _out0;
      _out0 = (this).createArrayOfZeroes((new BigInteger((this.cells).Length)) * (new BigInteger(2)));
      (this).cells = _out0;
      BigInteger _5_i;
      _5_i = BigInteger.Zero;
      while ((_5_i) < (_3_oldArrayLength)) {
        var _arr2 = this.cells;
        _arr2[(int)((_5_i))] = (_4_oldArray)[(int)(_5_i)];
        if ((_5_i) == ((_3_oldArrayLength) - (BigInteger.One))) {
          goto after_0;
        } else {
